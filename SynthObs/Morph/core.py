...
class physical_image():
    ...
    def __init__(self, X, Y, L, resolution = 0.1, Ndim = 100, smoothing = 'simple',centre=None):
...
class webbPSF():
    ...
    def __init__(self, filter, width, resampling_factor=1):
        ...
        elif filter in FLARE.filters.MIRI:
...
def observed_images(X, Y, fluxes, filters, cosmo, redshift=8, width=10., resampling_factor=1, smoothing=False, PSFs = False, show = False,centre=None):

    return {f: observed_image(X, Y, fluxes[f], f, cosmo, redshift, width, resampling_factor, smoothing, PSFs[f], show, centre) for f in filters}
...
class observed_image():
    ...
    def __init__(self, X, Y, flux, filter, cosmo, redshift=8, width=10., resampling_factor=1, smoothing=False, PSF = None, show = False,centre=None):
        """
        ...
        :param width: Width of the image along a single axis (this is approximate since images must be odd in dimension)
        :param resampling_factor: The integer amount of resampling done to increase resolution. (int)
        ##CHANGED. :param smoothed: Boolean, whether to apply smoothing.
        :param PSF: Instance of the webbPSFs object for the desired filter .
        :param show: Boolean, whether to show images.
        :param centre: 2D vector describing the centre of the image. If not specified, uses median of particle positions
        """
    ...
    def smoothimg(self, F):
        """ The same smoothing used for physical_image, as smoothimg_original gives weird artifacts.
        :param F: The flux array for the current filter
        :return: Image array
        """
        gsmooth_img = np.zeros((self.Ndim, self.Ndim))
        ...
        if self.smoothing == 'simple_old':
            Gx, Gy = np.meshgrid(np.linspace(-self.width/2., self.width/2., self.Ndim), np.linspace(-self.width/2., self.width/2., self.Ndim))
            # --- can probably replace this with the unsmoothed but convolved by a gaussian
            r = 0.1
            for x,y,l in zip(self.X_arcsec, self.Y_arcsec, F):
                gauss = np.exp(-(((Gx - x)**2 + (Gy - y)**2)/ ( 2.0 * r**2 ) ) )  
                sgauss = np.sum(gauss)
                if sgauss > 0: gsmooth_img += l*gauss/sgauss
        elif self.smoothing == 'simple': 
            Gx, Gy = np.meshgrid(np.linspace(-(self.width+self.resolution)/2., (self.width+self.resolution)/2., self.Ndim+1), np.linspace(-(self.width+self.resolution)/2., (self.width+self.resolution)/2., self.Ndim+1))
            r = 0.1
            gauss = np.exp(-((Gx**2 + Gy**2)/ ( 2.0 * r**2 ) ) )  
            gauss /= np.sum(gauss)
            g = np.linspace(-self.width/2.,self.width/2.,self.Ndim)
            for x,y,l in zip(self.X_arcsec, self.Y_arcsec, F):
                i, j = (np.abs(g - x)).argmin(), (np.abs(g - y)).argmin()
                gsmooth_img[j,i] += l
            gsmooth_img = convolve_fft(gsmooth_img, gauss)
        elif self.smoothing == 'adaptive':
            Gx, Gy = np.meshgrid(np.linspace(-self.width/2., self.width/2., self.Ndim), np.linspace(-self.width/2., self.width/2., self.Ndim))
            tree = cKDTree(np.column_stack([self.X_arcsec, self.Y_arcsec]), leafsize=16, compact_nodes=True, copy_data=False, balanced_tree=True)
            nndists, nninds = tree.query(np.column_stack([self.X_arcsec, self.Y_arcsec]), k=7, n_jobs=-1) # k = nth nearest neighbour
